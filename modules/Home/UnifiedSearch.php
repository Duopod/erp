...
function getRecordSummary(SugarBean $bean)
{
    ...
    foreach ($listViewDefs[$bean->module_dir] as $key => $entry) {
        ...
        if (in_array($key, array('date_entered',
            'date_modified',
            'name'))
        ) {
            continue;
...
{
    $output = preg_replace(array('/(?<=[^A-Z])([A-Z])/',
        '/(?<=[^0-9])([0-9])/'), $sep . '$0', $input);
...
function getPaginateHTML($queryString, $start, $amount, $total)
{
    ...
    <td class="paginationChangeButtons" align="right" nowrap="nowrap" width="1%">
        <form action='index.php'>
            ...
            <span class="pageNumbers">(<?php echo $total ? $start + 1 : 0; ?>
                - <?php echo min($start + $amount, $total); ?> of <?php echo $total; ?>)</span>
